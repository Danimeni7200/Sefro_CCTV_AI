...
ipcMain.handle('discover-camera', async (_evt, ip, user, pass, brand) => {
    try {
        const url = `http://127.0.0.1:8088/discover?ip=${encodeURIComponent(ip)}&user=${encodeURIComponent(user)}&password=${encodeURIComponent(pass)}&brand=${encodeURIComponent(brand || '')}`
